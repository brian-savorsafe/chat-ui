...
export default function useSpeech(onSpeech: (transcript: string) => void, allowSpeech?: AllowSpeech) {
  ...
  const [controlledRecording, onControlledRecordingChange, speechInControlled] = React.useMemo(() => {
    if (typeof allowSpeech === 'object') {
      return [allowSpeech.recording, allowSpeech.onRecordingChange, typeof allowSpeech.recording === 'boolean'] as const;
    }
  ...
  React.useEffect(() => {
    if (typeof navigator !== 'undefined' && 'permissions' in navigator) {
      ...
      (navigator as any).permissions.query({ name: 'microphone' }).then((permissionStatus: PermissionStatus) => {
